...
function renderResources() {
  ...
  resources.forEach((r) => {
    ...
    el.innerHTML = `<div class="title">${escapeHtml(
      r.title
    )}</div><div class="meta">${escapeHtml(r.description || "")}</div>`;
    if (r.link) {
...
document.getElementById("cta-quiz").addEventListener("click", function () {
  // direct open quiz page
...
document.getElementById("chat-btn").addEventListener("click", () => {
  ...
  if (existing)
    return (existing.animate([{ opacity: 1 }, { opacity: 0 }], {
      duration: 1200,
      iterations: 1,
